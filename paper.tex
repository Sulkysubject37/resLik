...
\begin{itemize}
    ...
    \item The controller answers: ``What should be done about it?''
\end{itemize}
...
A critical innovation of RLCS is the rigorous definition of how multiple sensors coexist. Naive approaches often use weighted averaging or majority voting, which are identified here as anti-patterns.
...
\begin{figure}[h]
    ...
    \caption{Multi-Sensor Response to Different Failure Modes. A simulated scenario shows how ResLik detects gradual drift (Population failure) while TCS remains stable, and how TCS detects sudden shock (Temporal failure). Agreement detects sensor conflict independently. This orthogonality allows nuanced control decisions.}
