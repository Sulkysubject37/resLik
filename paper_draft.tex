...
\begin{figure}[h]
    ...
    \begin{verbatim}
    +-------------+       +-------------+       +-----------------+
    |   Encoder   | ----> | Sensor Array| ----> | Control Surface |
    | (Inference) |   z   |  (Sensing)  |   d   |   (Signaling)   |
    +-------------+       +-------------+       +-----------------+
                                                         | u
                                                         v
                                                +-----------------+
...
\begin{itemize}
    ...
    \item \textbf{Result}: The system automatically tagged drifting data for retraining (\texttt{PROCEED\_WITH\_TAG}) while dropping corrupted data (\texttt{ABSTAIN}), automating data hygiene.
\end{itemize}
